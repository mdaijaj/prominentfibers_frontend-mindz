...
import { OtherDeducDilogComponent } from './other-deduc-dilog/other-deduc-dilog.component';
import { CellValueChangedEvent, GridApi, GridReadyEvent } from 'ag-grid-community';
import { OtherDeducActionComponent } from './other-deduc-action/other-deduc-action.component';
...
this.ItrDeductionForm.patchValue({
  ...
  public columnDefs = [
    ...
    {
      headerName: 'Actions',
      field: 'other_deduction_Id', sortable: true,
      resizable: true,
      wrapHeaderText: true,
      autoHeaderHeight: true,
      flex:1,
      minWidth:150,
      cellRenderer: OtherDeducActionComponent,
      cellRendererParams: {
        className: 'mat-blue',
  ...
  onCellValueChanged(event: CellValueChangedEvent) {
    ...
    let id2 = event.data.other_deduction_Id;
  let val = event.newValue
    ...
    let sta = event.data;
      ...
      this.hrServies.updateItrOtherDeduct(id2, sta).subscribe(
        (res: any) => {
  ...
  onCellClicked(e: any) {
    ...
    if (e.column.colId === "other_deduction_update") return;
    const dialogRef = this.dialog.open(OtherDeducDilogComponent, { width: '400px', data: { id: e.data.other_deduction_Id}});
  ...
  getAllOtherDeduc(){
    this.hrServies.getAllOtherDeduc().subscribe((res:any)=>{
