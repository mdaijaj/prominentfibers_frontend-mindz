...
@Component({
  selector: 'app-esi-create',
  templateUrl: './esi-create.component.html',
  styleUrls: ['./esi-create.component.scss']
})
export class EsiCreateComponent {
  selectedMonth = 'Jan'
  ...
  ngOnInit() {
    ...
    if (this.pf_id) {
      this.hrServies.getPfById(this.pf_id).subscribe((res: any) => {
        ...
        if(this.singlePfData.frontComponent == "LWF" || this.singlePfData.frontComponent=="PT"){
        ...
        this.pfForm.patchValue({
          addGroup_name: this.singlePfData.addGroup_nameById,
  ...
  onSubmit() {
    ...
    this.hrServies.addPfCreate(data).subscribe((res: any) => {
      ...
      if (res.code == 200) {
        ...
        this.route.navigate(['master/hrms/payroll/payroll-master/payroll-component/esi-list']);
      }
