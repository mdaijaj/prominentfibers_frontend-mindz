...
@Component({
  selector: 'app-draft-travel-list',
  templateUrl: './draft-travel-list.component.html',
  styleUrls: ['./draft-travel-list.component.scss']
})
export class DraftTravelListComponent {
  ...
  // this.leadService.getTravelRequest().subscribe((res:any) => {
    
    this.leadService.getAllTravelDraft().subscribe((res:any) => {
    // let travelName:any=[]
...
// ]
  ...
  openCreate() {
    this.route.navigate(
      ['master/expense-main/travel-request/travel-request/task-order-travel'],
  ...
  onCellClicked(e:any){
    ...
    const dialogRef = this.dialog.open(AppListDialogComponent,{width:'40%',data:{verifyId:e.data.candidtaes_v_Id}});
