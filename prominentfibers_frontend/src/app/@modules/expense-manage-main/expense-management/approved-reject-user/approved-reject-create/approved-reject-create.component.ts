...
@Component({
  selector: 'app-approved-reject-create',
  templateUrl: './approved-reject-create.component.html',
  styleUrls: ['./approved-reject-create.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class ApprovedRejectCreateComponent {
  ...
  constructor(private fb: FormBuilder,
    ...
    // private date_pipe:DatepipePipe
...
patchTravelTicket(){
  ...
  public columnDefs = [
    ...
    
    {
      ...
      cellClass: "grid-cell-centered",
      minWidth: 150,
      // valueFormatter:(params:any)=> params.data.attachedBill?'download file':'File not found',
  ...
  submitForm() {
    ...
    Swal.fire({
      title: 'Are you sure to Update this Expense ?',
  ...
  updateExpenseDetail() {
    ...
    let data = {
      ...
      quantity:Number(val.quantity),
      attachedBill:this.fileDetails.file
    }
  ...
  expenceDetailsGetByCopy() {
    this._itteketService.getByExpenseDetailsIdCopy(this.expenseDetailsId).subscribe((res: any) => {
      ...
      if(res.data?.attachedBill.includes('attachedBill')){
// return res.data.attachedBill=this.pdfFile
       return  this.attachedbill='../../../../../../assets/icons/pdfimg.png'
      }
      else{
  ...
  cancel() {
    this.route.navigate(['master/expense-main/expense-managment/aproved-reject-user'])
  }
