...
@Component({
  selector: 'app-book-ticket-travel-request',
  templateUrl: './book-ticket-travel-request.component.html',
  styleUrls: ['./book-ticket-travel-request.component.scss']
})
export class BookTicketTravelRequestComponent {
  leadForm: FormGroup;
  jobType: any;
  candidateArray: any;
  interviewGetId:any;
  ...
  empId: string | null;
  ticketMondtory: boolean=false;
  ticketMondtoryTick: boolean=false;
...
this.getBymultipleTravel();
    ...
    // this.getExpenseDetail();
      ...
      this._itteketService.sayMessages(this.lead_id);
...
// status:Booked_Ticket
  ...
  bookForm() {
    ...
    if(this.ticketMondtory || this.ticketMondtoryTick){
// alert("1st upload file");
this.toast.warning("Please Attach Ticket And Invoice")
  ...
  return
    ...
    //   return
          // }
    Swal.fire({
      title: 'Are you sure to Booked Ticket this Travler ?',
    ...
    this.leadService.updateTravel(this.lead_id, data).subscribe((res:any) => {
      ...
      // this._itteketService.send_data.next(this.lead_id);
...
if(res.result[i]?.attach_invoice===null){
this.ticketMondtory=true;
}
if(res.result[i]?.attach_ticket===null){
  this.ticketMondtoryTick=true;

}
        }
        console.log(this.ticketMondtory,'this.ticketMondtory');
        console.log(this.ticketMondtoryTick,'this.ticketMondtoryTick=true');
    ...
    onCellClickedCopy(e:any){
      console.log(e,'eee tikct copy');
        ...
        = this.dialog.open(ExpenseDialogComponent, {
          ...
          data: { confirmTicketId:e.data.ticket_id}
