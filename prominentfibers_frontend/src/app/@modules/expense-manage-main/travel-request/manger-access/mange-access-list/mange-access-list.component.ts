...
@Component({
  selector: 'app-mange-access-list',
  templateUrl: './mange-access-list.component.html',
  styleUrls: ['./mange-access-list.component.scss']
})
export class MangeAccessListComponent {
...
this.empId=localStorage.getItem('EmpMainId');
  ...
  this._itTecting.getAllMangerList(this.empId).subscribe((res:any) => {
...
this.rowData=filterData.reverse();
  })  
...
// ]
  ...
  onCellClicked(e:any){
    console.log(e,'eee task order');
    
    const dialogRef = this.dialog.open(ExpenseDialogComponent,{width:'45%',data:{expenseId:e.data.expense_id}});
