...
import { VariableActionComponent } from './variable-action/variable-action.component';
import { VariableDilogComponent } from './variable-dilog/variable-dilog.component';
...
export class VariableComponent {
  ...
  public columnDefs = [
    ...
    },
    {
      ...
      flex: 1,
      minWidth: 150,
      cellRenderer: VariableActionComponent,
      cellRendererParams: {
        className: 'mat-blue',
  ...
  onFilterTextBoxChanged(e: any) {
    ...
    let data = this.gridApi.setQuickFilter(e.value);

  }

  onCellValueChanged(event: CellValueChangedEvent) {
    // let id1 = event.data.states_id;
    let id2 = event.data.other_variable_Id;
  let val = event.newValue
    ...
    let sta = event.data;
      ...
      this.hrServies.updateItrOtherVar(id2, sta).subscribe(
        (res: any) => {
  ...
  onCellClicked(e: any) {
    ...
    if (e.column.colId === 'other_variable_update') return;
    const dialogRef = this.dialog.open(VariableDilogComponent, {
      width: '400px',
      data: { id: e.data.other_variable_Id },
    });
  ...
  getAllVariable() {
    this.hrServies.getAllVariable().subscribe((res: any) => {
